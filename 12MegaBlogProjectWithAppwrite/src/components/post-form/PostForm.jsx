...
function PostForm({ post }) {
  ...
  React.useEffect(() => {
    const subscription = watch((value, name) => {
      if (name === 'title') {
        setValue('slug', slugTransformer(value.title, { shouldValidate: true }));
      }
    });
  ...
  }, [watch, slugTransformer, setValue]);
    ...
    <form onSubmit={handleSubmit(submit)} className="flex flex-wrap">
      <div className="w-2/3 px-2">
        ...
        <Input
          ...
          onInput={(e) => {
            setValue('slug', slugTransformer(e.currentTarget.value), { shouldValidate: true });
          }}
        />
      ...
      <div className="w-1/3 px-2">
        ...
        {post && (
          <div className="w-full mb-4">
            <img src={fileService.getFilePreview(post.featuredImage)} alt={post.title} className="rounded-lg" />
          </div>
        )}
        <Select
